...
const ModalCategory =({idCategory, onUpdate, onCreate, children}) =>{
    ...
    const submitData = (e) => {
        ...
        setSaving(true);
        if (idCategory) {
          BaseApi.put(`/categorias/${idCategory}`, {
            nome: name,
          }).then(res => {
            setSaving(false);
            setShowModal(false);
      ...
      const requestData = () => {
        setLoading(true);
        BaseApi.get(`/users${idCategory ? `/${idCategory}` : ''}`).then(res => {
          let data = res.data;
          setName(data.nome);
          setLoading(false);
          setSaving(false);
      ...
      {!children && (
        <Tooltip text={idCategory ? "Edit Catgeory" : "Create Category"}>
          <button
